...
def move_player(player, moves):
    ...
    i,j = convert_position_to_indices(position)
    position += moves
    i,j = convert_position_to_indices(position)
...
def get_entity_value(p):
    ...
    for i in range(n_components):
        if (snakes[i][1] == p):
            result = snakes[i][0] - snakes[i][1]
        if (ladders[i][0] == p):
            result = ladders[i][1] - p
    return result
